...
def calcular_metricas_dividendo(t, hist_5y):
    ...
    MÉTODO: usa uma janela móvel de 12 meses (TTM — trailing twelve months) em vez
    de somar por ano civil (Jan-Dez). Isto é importante porque muitas empresas
    (ex: PepsiCo) aumentam o dividendo A MEIO do ano civil — somar por ano civil
    mistura trimestres com a taxa antiga e a nova taxa em anos diferentes, o que
    pode fazer um dividendo que só sobe parecer instável ou a descer. Comparar
    sempre "os últimos 12 meses" com "os 12 meses um ano antes" resolve isto,
    seja qual for o mês em que a app for consultada."""
    try:
        divs = t.dividends
        ...
        # TTM: soma móvel dos dividendos pagos nos últimos 365 dias, avaliada em cada data de pagamento
        ttm = divs.rolling("365D").sum()
        if ttm.empty:
            return np.nan, np.nan, np.nan

        data_atual = ttm.index[-1]
        ttm_atual = ttm.iloc[-1]

        # ---- anos consecutivos de aumento: recua ano a ano comparando TTM ----
        anos_consecutivos = 0
        data_ref, ttm_ref = data_atual, ttm_atual
        for _ in range(25):  # limite de segurança (25 anos é mais que suficiente)
            data_alvo = data_ref - pd.DateOffset(years=1)
            pos = ttm.index.searchsorted(data_alvo)
            if pos == 0:
                break
            ttm_anterior = ttm.iloc[pos - 1]
            if ttm_ref >= ttm_anterior * 0.999:  # pequena tolerância a arredondamentos
                anos_consecutivos += 1
                data_ref, ttm_ref = data_alvo, ttm_anterior
            else:
                break

        # ---- CAGR do dividendo: TTM atual vs. TTM de há ~5 anos ----
        dividend_cagr_5y = np.nan
        data_alvo_5y = data_atual - pd.DateOffset(years=5)
        pos5 = ttm.index.searchsorted(data_alvo_5y)
        if pos5 > 0:
            ttm_5y_atras = ttm.iloc[pos5 - 1]
            if ttm_5y_atras and ttm_5y_atras > 0:
                dividend_cagr_5y = (ttm_atual / ttm_5y_atras) ** (1 / 5) - 1
