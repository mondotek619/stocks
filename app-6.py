...
PORTFOLIO = [
    ...
    {"nome": "McDonald's",       "ticker": "MCD",     "peso": 4,  "setor": "Consumo Discricionário"},
    {"nome": "Domino's Pizza",   "ticker": "DPZ",     "peso": 4,  "setor": "Consumo Discricionário"},
]
...
def obter_dados(ticker, tentativas=2):
    ...
    for tentativa in range(tentativas):
        try:
            ...
            # cálculo do "yield médio 5 anos"). Nenhum yield real de blue-chip ultrapassa
            # ~20-25% — qualquer resultado acima disso é claramente um erro de dados, seja
            # qual for a fonte, por isso o limite de sanidade aplica-se sempre, em cascata:
            # tenta o TTM, depois o dividendRate, depois o campo antigo do yfinance.
            def _valido(valor):
                return pd.notna(valor) and 0 < valor < 20

            dividend_yield = np.nan
            if pd.notna(ttm_dividendo_atual) and ttm_dividendo_atual > 0 and preco_atual:
                candidato = (ttm_dividendo_atual / preco_atual) * 100
                if _valido(candidato):
                    dividend_yield = candidato
            if not _valido(dividend_yield) and pd.notna(dividend_rate_bruto) and dividend_rate_bruto and preco_atual:
                candidato = (dividend_rate_bruto / preco_atual) * 100
                if _valido(candidato):
                    dividend_yield = candidato
            if not _valido(dividend_yield):
                ...
                if div_yield_raw is not None:
                    ...
                    if _valido(candidato):
                        dividend_yield = candidato
            if not _valido(dividend_yield) and pd.notna(yield_medio_5anos) and yield_medio_5anos > 0:
                # último recurso: usa o yield médio de 5 anos, que já provou ser fiável
                dividend_yield = yield_medio_5anos
            if not _valido(dividend_yield):
                dividend_yield = np.nan
            ...
            try:
                ...
                free_cashflow = fcf if fcf else np.nan
                shares_outstanding = shares_out if shares_out else np.nan
            ...
            # ---- Buyback Yield: recompras de ações ÷ capitalização bolsista ----
            # Usado para ajustar o Valor Intrínseco em empresas que devolvem valor
            # aos acionistas sobretudo via recompras (ex: Mastercard) em vez de
            # dividendo. Opcional — devolve 0.0 quando não há dados, nunca N/D.
            buyback_yield = calcular_buyback_yield(t, market_cap)

            # ---- Preços-alvo e recomendações dos analistas ----
            # Já vêm dentro do "info", sem pedido extra ao Yahoo Finance.
            ...
            numero_analistas = info.get("numberOfAnalystOpinions", np.nan)

            upside_analistas = np.nan
            if pd.notna(target_mean_price) and target_mean_price > 0 and preco_atual:
                upside_analistas = ((target_mean_price - preco_atual) / preco_atual) * 100
            ...
            return {
                ...
                "numero_analistas": numero_analistas,
                "upside_analistas": upside_analistas,
...
def calcular_metricas_dividendo(t, hist_5y):
    ...
    try:
        ...
        # ---- anos consecutivos: compara sempre com o pagamento mais recente como
        # âncora fixa (data_mais_recente - N anos), em vez de encadear os pontos já
        # aproximados de uma iteração para a seguinte — isso evitava que um pequeno
        # desvio (por causa da margem de tolerância) se fosse acumulando ano após
        # ano até "escapar" da janela de tolerância e parar cedo demais (ex: uma
        # empresa com 30 anos de aumentos a aparecer com só "1 ano").
        idx_referencia = len(valores) - 1
        data_mais_recente = datas[idx_referencia]
        valor_mais_recente = valores[idx_referencia]

        anos_consecutivos = 0
        idx_ano_anterior = idx_referencia
        valor_ano_anterior = valor_mais_recente
        for n in range(1, 61):  # limite de segurança (60 anos cobre até os Dividend Kings mais antigos)
            data_alvo = data_mais_recente - pd.DateOffset(years=n)
            idx_candidato = _pagamento_mais_proximo(data_alvo, idx_ano_anterior, margem_dias=45)
            if idx_candidato is None:
                break
            valor_candidato = valores[idx_candidato]
            if valor_ano_anterior >= valor_candidato * 0.999:  # tolerância a arredondamentos
                anos_consecutivos += 1
                idx_ano_anterior = idx_candidato
                valor_ano_anterior = valor_candidato
            else:
                break
    ...
    for i, ativo in enumerate(PORTFOLIO):
        ...
        if dados and "erro" not in dados:
            ...
            linha.update({
                ...
                "free_cashflow": np.nan, "shares_outstanding": np.nan, "buyback_yield": 0.0,
                "target_mean_price": np.nan, "target_high_price": np.nan, "target_low_price": np.nan,
                "recommendation_key": None, "recommendation_mean": np.nan, "numero_analistas": np.nan,
                "upside_analistas": np.nan,
            })
        linhas.append(linha)
...
# ------------------------------------------------------------------
# VALOR INTRÍNSECO (Dividend Discount Model — DUAS FASES)
# ------------------------------------------------------------------
# Versão de duas fases, mais robusta do que o modelo de Gordon simples (uma
# fase só) que a app usava antes:
#
#   FASE 1 (anos 1 a 10): usa a taxa de crescimento real da empresa (CAGR do
#   dividendo dos últimos 5 anos), limitada a um intervalo razoável [2%, 10%].
#   Como esta fase é temporária (só 10 anos, não perpétua), pode ser mais
#   generosa sem instabilidade.
#
#   FASE 2 (perpetuidade, a partir do ano 11): usa uma taxa de crescimento
#   terminal fixa e conservadora (3%, perto do crescimento nominal de longo
#   prazo da economia global) — nunca a taxa real da empresa. Isto evita a
#   armadilha clássica do modelo de uma fase só, em que g podia aproximar-se
#   de r e fazer o valor "explodir" matematicamente.
#
#   AJUSTE DE RECOMPRAS (buyback yield): empresas que devolvem dinheiro aos
#   acionistas sobretudo via recompra de ações (ex: Mastercard) em vez de
#   dividendo têm o "dividendo base" ajustado para cima, refletindo esse
#   valor devolvido por outra via. Isto aproxima o modelo do conceito de
#   "shareholder yield" (dividendo + recompras), mais completo do que olhar
#   só para o dividendo pago.
#
# ⚠️ Continua a ser uma ESTIMATIVA, não uma verdade:
#   - Só faz sentido para empresas com dividendo estável. Sem histórico
#     suficiente, o modelo devolve N/D em vez de um número enganador.
#   - É sensível às assunções — usa para comparar ações, não como preço-alvo.
#   - A Margem de Segurança aqui é calculada como % face ao PREÇO ATUAL (não
#     face ao valor intrínseco). Isto limita o lado "sobrevalorizado" a -100%
#     (nunca pode "explodir" para lá disso), mas o lado "subvalorizado" fica
#     sem limite superior — é uma escolha de apresentação mais intuitiva,
#     não uma correção ao problema de fundo do modelo.

def calcular_valor_intrinseco(row, taxa_desconto, g_terminal=0.03):
    dividend_rate = row.get("dividend_rate")
    cagr = row.get("dividend_cagr_5y")
    preco = row.get("preco")
    buyback_yield = row.get("buyback_yield") or 0.0

    if pd.isna(dividend_rate) or not dividend_rate or dividend_rate <= 0:
        return np.nan, np.nan
    if pd.isna(cagr):
        return np.nan, np.nan
    if taxa_desconto <= g_terminal:
        return np.nan, np.nan  # a taxa exigida tem sempre de exceder o crescimento terminal

    # Dividendo base ajustado pelas recompras de ações (shareholder yield)
    dividendo_base = dividend_rate * (1 + buyback_yield)

    # Fase 1: taxa de crescimento explícita, limitada a [2%, 10%]
    g1 = max(0.02, min(cagr, 0.10))

    valor_presente_fase1 = 0.0
    dividendo_ano = dividendo_base
    for ano in range(1, 11):
        dividendo_ano = dividendo_ano * (1 + g1)
        valor_presente_fase1 += dividendo_ano / ((1 + taxa_desconto) ** ano)

    # Fase 2: valor terminal no fim do ano 10, descontado para hoje
    dividendo_ano11 = dividendo_ano * (1 + g_terminal)
    valor_terminal_ano10 = dividendo_ano11 / (taxa_desconto - g_terminal)
    valor_presente_terminal = valor_terminal_ano10 / ((1 + taxa_desconto) ** 10)

    valor_intrinseco = valor_presente_fase1 + valor_presente_terminal

    if pd.isna(preco) or preco <= 0 or pd.isna(valor_intrinseco):
        margem_seguranca = np.nan
    else:
        # % face ao preço atual (não ao valor intrínseco) — ver nota acima
        margem_seguranca = ((valor_intrinseco - preco) / preco) * 100

    return valor_intrinseco, margem_seguranca


def calcular_buyback_yield(t, market_cap):
    """Estima o buyback yield (recompras de ações ÷ capitalização bolsista) a
    partir da demonstração de fluxos de caixa anual. Devolve 0.0 (não 
    N/D) quando não há dados — recompras são um extra opcional, a sua
    ausência não deve impedir o cálculo do valor intrínseco."""
    try:
        cf = t.cashflow
        if cf is None or cf.empty or not market_cap:
            return 0.0
        for nome in ["Repurchase Of Capital Stock", "Repurchase Of Stock"]:
            if nome in cf.index:
                valor = cf.loc[nome].iloc[0]
                if pd.notna(valor):
                    return abs(valor) / market_cap  # valor vem tipicamente negativo (saída de caixa)
        return 0.0
    except Exception:
        return 0.0
...
# ------------------------------------------------------------------
# VALOR INTRÍNSECO VIA FREE CASH FLOW (DCF simplificado)
# ------------------------------------------------------------------
# O Dividend Discount Model só "vê" o dinheiro que sai como dividendo. Isso
# subestima MUITO empresas de baixo payout (ex: Mastercard, Ecolab), que
# reinvestem a maior parte do lucro em crescimento e recompras de ações —
# é aí que está grande parte do seu valor real, não no dividendo pago.
#
# Este modelo usa o Free Cash Flow (o dinheiro que a empresa gera de facto,
# depois de pagar as despesas do negócio, ANTES de decidir o que fazer com
# ele — pagar dividendo, recomprar ações, ou reinvestir) em vez do dividendo.
# Funciona para qualquer empresa lucrativa, mesmo as que não pagam dividendo.
#
# Fórmula: Valor por Ação = (FCF por Ação × (1+g)) ÷ (r - g)
#   g = crescimento estimado, com os mesmos limites conservadores do DDM
#       (máximo 5%, spread mínimo de 3 pontos percentuais face a r)

def calcular_valor_intrinseco_fcf(row, taxa_desconto):
    fcf_total = row.get("free_cashflow")
    shares_out = row.get("shares_outstanding")
    preco = row.get("preco")

    if pd.isna(fcf_total) or not fcf_total or fcf_total <= 0:
        return np.nan, np.nan
    if pd.isna(shares_out) or not shares_out or shares_out <= 0:
        return np.nan, np.nan

    fcf_por_acao = fcf_total / shares_out

    # Crescimento: usa a média do crescimento de receita e de resultados
    # (mais adequado aqui do que o CAGR do dividendo, já que estamos a
    # avaliar o negócio como um todo, não só a política de dividendos)
    candidatos_g = [row.get("revenue_growth"), row.get("earnings_growth")]
    candidatos_g = [g for g in candidatos_g if pd.notna(g)]
    g = np.mean(candidatos_g) if candidatos_g else 0.03  # fallback conservador

    g_ajustado = min(g, 0.05, taxa_desconto - 0.03)
    g_ajustado = max(g_ajustado, -0.05)

    d1 = fcf_por_acao * (1 + g_ajustado)
...
def aplicar_valor_intrinseco_fcf(df, taxa_desconto):
    """Adiciona as colunas de Valor Intrínseco via FCF e respetiva Margem de Segurança."""
    resultados = df.apply(lambda r: calcular_valor_intrinseco_fcf(r, taxa_desconto), axis=1)
    df = df.copy()
    df["valor_intrinseco_fcf"] = resultados.apply(lambda x: x[0])
    df["margem_seguranca_fcf"] = resultados.apply(lambda x: x[1])
    return df
...
with st.spinner("A carregar cotações..."):
    ...
    df = aplicar_valor_intrinseco_fcf(df, taxa_desconto)
...
# TABS
# ------------------------------------------------------------------
tab_resumo, tab_alertas, tab_alocacao, tab_ratings, tab_dividendos, tab_intrinseco, tab_analistas, tab_reforco, tab_detalhe = st.tabs(
    ["📋 Resumo", "🚨 Alertas", "🥧 Alocação", "🏆 Quant Ratings", "💰 Dividend Safety",
     "🧮 Valor Intrínseco", "🎯 Analistas", "💎 Oportunidades de Reforço", "🔍 Detalhe por Ativo"]
)
...
with tab_intrinseco:
    st.subheader("Valor Intrínseco — Dividend Discount Model (2 Fases)")
    st.caption(
    ...
    with st.expander("❓ Como funciona e principais limitações (lê antes de confiar nos números)"):
        st.markdown("""
**Modelo de duas fases** (mais robusto que um Gordon Growth simples de fase única):

- **Fase 1 (anos 1-10):** usa o CAGR real do dividendo dos últimos 5 anos, limitado a um intervalo de **2% a 10%**.
- **Fase 2 (perpetuidade, a partir do ano 11):** usa uma taxa de crescimento terminal fixa e conservadora de **3%** — nunca a taxa de crescimento real da empresa. Isto evita a "explosão" matemática que acontece quando o crescimento real se aproxima da taxa de retorno exigida.
- **Ajuste de recompras de ações:** o dividendo-base é ajustado para cima consoante o *buyback yield* da empresa (recompras ÷ capitalização bolsista), para refletir empresas que devolvem valor sobretudo via recompras (ex: Mastercard) em vez de dividendo.

**⚠️ Limitações importantes — isto é uma estimativa, não uma verdade:**
- Mesmo com o ajuste de recompras, **pode continuar a subestimar empresas de baixo payout** que reinvestem muito do lucro sem grandes recompras nem dividendo. Para essas, a tabela de Valor Intrínseco via Free Cash Flow (mais abaixo) costuma ser mais fiável.
- Só faz sentido para empresas com **histórico de dividendos estável**. Ações sem dividendo aparecem como "N/D".
- É **muito sensível** às assunções: muda a taxa de retorno na barra lateral e repara como os valores mudam. Usa isto para comparar ações entre si, não como um preço-alvo exato.

**Margem de Segurança** = quanto a ação pode subir (positivo) ou descer (negativo) até ao valor intrínseco estimado, **em % do preço atual**. Ao contrário de calcular em % do valor intrínseco, isto limita o lado "sobrevalorizado" a -100% — mas não corrige, só disfarça visualmente, a limitação de fundo do modelo referida acima.
        """)
    ...
    if intrinseco_df.empty:
        ...
        tabela_intrinseco = pd.DataFrame({
            ...
            "Fiabilidade do Modelo": intrinseco_df["payout_ratio"].map(
                lambda p: "⚠️ Baixa (payout baixo)" if pd.notna(p) and p < 30 else "✅ Normal"
            ),
        })
        ...
        n_baixa_fiabilidade = (intrinseco_df["payout_ratio"] < 30).sum()
        if n_baixa_fiabilidade > 0:
            st.warning(
                f"⚠️ {n_baixa_fiabilidade} ativo(s) têm payout ratio abaixo de 30% (ex: Mastercard, "
                "Ecolab). O modelo já ajusta pelo buyback yield (recompras de ações), mas pode continuar "
                "a subestimar o valor real destas empresas se também reinvestirem muito lucro sem "
                "grandes recompras. Uma margem muito negativa nestes casos nem sempre significa que a "
                "ação esteja cara — pode só significar que este modelo específico capta pouco do valor "
                "total gerado. Para estas ações, confia mais na tabela de Valor Intrínseco via Free Cash "
                "Flow (mais abaixo) ou no Quant Rating (aba 🏆)."
            )
    ...
    st.caption(
        ...
        "uma margem de segurança positiva não vale muito se o dividendo estiver em risco (nota D), e uma "
        "margem muito negativa não vale muito se o payout for baixo (ver aviso acima — ou usa a tabela "
        "abaixo, que resolve exatamente esse problema)."
    )

    st.divider()
    st.subheader("💵 Valor Intrínseco via Free Cash Flow (DCF simplificado)")
    st.caption(
        "Este modelo usa o dinheiro que a empresa gera de facto (Free Cash Flow), em vez de olhar só "
        "para o dividendo pago. Funciona bem mesmo para empresas de baixo payout (ex: Mastercard, "
        "Ecolab), onde o Dividend Discount Model acima é pouco fiável."
    )

    with st.expander("❓ Como funciona este modelo"):
        st.markdown("""
**Fórmula:** `Valor por Ação = (FCF por Ação × (1 + g)) ÷ (Taxa de Retorno Exigida − g)`

- **FCF por Ação** = Free Cash Flow total da empresa ÷ nº de ações em circulação
- **g** = média do crescimento de receita e de resultados, com os mesmos limites conservadores do modelo acima (máximo 5%, spread mínimo de 3 pontos percentuais)

**Vantagem sobre o Dividend Discount Model:** capta o valor gerado pelo negócio como um todo — dividendos, recompras de ações e reinvestimento — não só o que é pago diretamente ao acionista.

**Continua a ser uma estimativa**, sujeita às mesmas limitações gerais de qualquer modelo deste tipo: sensível às assunções de crescimento e taxa de retorno, assume uma trajetória de crescimento constante que raramente se verifica exatamente assim, e o Free Cash Flow pode variar bastante de ano para ano (ex: em anos de investimento pesado).
        """)

    fcf_df = df.copy()
    fcf_df = fcf_df.dropna(subset=["valor_intrinseco_fcf"])

    if fcf_df.empty:
        st.info("Nenhum ativo tem dados de Free Cash Flow suficientes para aplicar este modelo.")
    else:
        fcf_df = fcf_df.sort_values("margem_seguranca_fcf", ascending=False)

        tabela_fcf = pd.DataFrame({
            "Ticker": fcf_df["ticker"],
            "Nome": fcf_df["nome"],
            "Preço Atual": fcf_df.apply(
                lambda r: f"{r['preco']:.2f} {r['moeda']}" if pd.notna(r["preco"]) else "N/D", axis=1),
            "Valor Intrínseco (FCF)": fcf_df["valor_intrinseco_fcf"].map(lambda x: f"{x:.2f}"),
            "Margem de Segurança": fcf_df["margem_seguranca_fcf"].map(formata_pct),
        })

        styled_fcf = tabela_fcf.style.map(cor_margem, subset=["Margem de Segurança"])
        st.dataframe(styled_fcf, use_container_width=True, height=500, hide_index=True)

        n_sem_fcf = len(df) - len(fcf_df)
        if n_sem_fcf > 0:
            st.caption(
                f"ℹ️ {n_sem_fcf} ativo(s) não aparecem por não terem dados de Free Cash Flow "
                "disponíveis no yfinance."
            )

# ---------------- TAB ANALISTAS ----------------
with tab_analistas:
    st.subheader("🎯 Potencial Segundo os Analistas de Wall Street")
    st.caption(
        "Preços-alvo médios e recomendações publicadas por analistas que cobrem cada ação, "
        "recolhidos pelo Yahoo Finance. Mostra o quanto cada ação pode subir (ou descer) até "
        "ao preço-alvo médio, segundo o consenso do mercado."
    )

    with st.expander("❓ Como interpretar e principais limitações"):
        st.markdown("""
**Preço-Alvo Médio (Mean Target)** — a média dos preços-alvo publicados por todos os analistas que cobrem a ação, tipicamente com horizonte de 12 meses.

**Preço-Alvo Mín/Máx** — o intervalo entre a estimativa mais pessimista e a mais otimista. Um intervalo muito largo é, por si só, um sinal de que há grande incerteza ou desacordo sobre a empresa.

**Recomendação** — o consenso entre Compra Forte, Compra, Manter, Vender e Venda Forte.

**⚠️ Limitações importantes:**
- Analistas trabalham tipicamente para bancos de investimento, que têm frequentemente outras relações comerciais com as empresas que cobrem — isto pode (não sempre, mas por vezes) enviesar as recomendações para cima.
- Há um efeito de "manada" bem documentado: analistas tendem a seguir-se uns aos outros e a ajustar os preços-alvo *depois* do mercado já ter reagido, não antes.
- Preços-alvo são revistos com frequência — um valor "desatualizado" (poucos analistas, cobertura antiga) é menos fiável.
- Isto é **uma opinião de mercado**, não uma previsão garantida. Usa em conjunto com o Quant Rating e o Valor Intrínseco, nunca isoladamente.
        """)

    analistas_df = df.copy()
    analistas_df = analistas_df.dropna(subset=["upside_analistas"])

    if analistas_df.empty:
        st.info("Nenhum ativo tem cobertura de analistas suficiente disponível no momento.")
    else:
        analistas_df = analistas_df.sort_values("upside_analistas", ascending=False)

        mapa_recomendacao = {
            "strong_buy": "Compra Forte", "buy": "Compra", "hold": "Manter",
            "sell": "Venda", "strong_sell": "Venda Forte", "underperform": "Abaixo do Mercado",
            "outperform": "Acima do Mercado", "none": "N/D",
        }

        tabela_analistas = pd.DataFrame({
            "Ticker": analistas_df["ticker"],
            "Nome": analistas_df["nome"],
            "Preço Atual": analistas_df.apply(
                lambda r: f"{r['preco']:.2f} {r['moeda']}" if pd.notna(r["preco"]) else "N/D", axis=1),
            "Alvo Médio": analistas_df["target_mean_price"].map(lambda x: f"{x:.2f}" if pd.notna(x) else "N/D"),
            "Alvo Mín-Máx": analistas_df.apply(
                lambda r: f"{r['target_low_price']:.0f}-{r['target_high_price']:.0f}"
                if pd.notna(r["target_low_price"]) and pd.notna(r["target_high_price"]) else "N/D", axis=1),
            "Upside/Downside": analistas_df["upside_analistas"].map(formata_pct),
            "Recomendação": analistas_df["recommendation_key"].map(
                lambda k: mapa_recomendacao.get(k, "N/D") if k else "N/D"),
            "Nº Analistas": analistas_df["numero_analistas"].map(
                lambda x: f"{int(x)}" if pd.notna(x) else "N/D"),
        })
        ...
        def cor_recomendacao(val):
            cores = {
                "Compra Forte": "#00c853", "Acima do Mercado": "#00c853", "Compra": "#7cd992",
                "Manter": "#ffb300", "Venda": "#ff5252", "Venda Forte": "#ff5252",
                "Abaixo do Mercado": "#ff5252",
            }
            cor = cores.get(val, "")
            return f"color: {cor}; font-weight: 600;" if cor else ""

        styled_analistas = (
            tabela_analistas.style
            .map(cor_upside, subset=["Upside/Downside"])
            .map(cor_recomendacao, subset=["Recomendação"])
        )
        st.dataframe(styled_analistas, use_container_width=True, height=600, hide_index=True)

        n_sem_cobertura = len(df) - len(analistas_df)
        if n_sem_cobertura > 0:
            st.caption(
                f"ℹ️ {n_sem_cobertura} ativo(s) não aparecem na tabela por não terem cobertura de "
                "analistas suficiente disponível no Yahoo Finance."
            )

        top3_upside = analistas_df.head(3)
        if not top3_upside.empty:
            st.divider()
            st.markdown("**🥇 Maior upside segundo os analistas**")
            for _, r in top3_upside.iterrows():
                st.markdown(f"""
                <div class="alert-card-green">
                    <b>{r['ticker']} — {r['nome']}</b><br>
                    Alvo médio: {r['target_mean_price']:.2f} {r['moeda']} (upside de {formata_pct(r['upside_analistas'])}) ·
                    {mapa_recomendacao.get(r['recommendation_key'], 'N/D')} ·
                    {int(r['numero_analistas']) if pd.notna(r['numero_analistas']) else 'N/D'} analistas
                </div>
                """, unsafe_allow_html=True)
