...
def obter_dados(ticker, tentativas=2):
    ...
    for tentativa in range(tentativas):
        try:
            ...
            # ---- Dividend Yield: calculado diretamente (dividendo anual ÷ preço) ----
            # Evita confiar no campo "dividendYield" do yfinance, cujo formato
            # (fração vs. já-em-percentagem) tem sido inconsistente consoante o
            # ticker/versão — isso já causou casos absurdos como um yield de 80%
            # numa ação que na realidade paga ~1,5-2% (ex: Caterpillar).
            dividend_rate_bruto = info.get("dividendRate", np.nan)
            if pd.notna(dividend_rate_bruto) and dividend_rate_bruto and preco_atual:
                dividend_yield = (dividend_rate_bruto / preco_atual) * 100
            else:
                # fallback: usa o campo do yfinance, mas descarta valores implausíveis
