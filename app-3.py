...
def calcular_metricas_dividendo(t, hist_5y):
    ...
    try:
        ...
        # Ignora SEMPRE o ano corrente na comparação de anos completos — mesmo em
        # dezembro, o ano pode não ter todos os pagamentos registados ainda no
        # yfinance. Comparar um ano parcial com anos completos fazia parecer que
        # o dividendo tinha descido (ex: PepsiCo a aparecer com CAGR negativo e
        # "0 anos consecutivos", quando na realidade só faltava pagar o resto do ano).
        agora = pd.Timestamp.now()
        if not dividendos_anuais.empty and dividendos_anuais.index[-1].year == agora.year:
